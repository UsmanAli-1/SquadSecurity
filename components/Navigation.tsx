...
export default function Navigation() {
  ...
  return (
    <nav className="sticky top-5  z-50 flex justify-center mt-2.5 px-4">
            ...
            {navLinks.map((link) => {
              ...
              return (
                ...
                >
                  ...
                  {/* Animated Underline */}
                  <span
                    className={`absolute bottom-0 left-0 h-0.5 bg-[#1F3D2B] transition-all duration-300 ease-out ${
                      isActive
                        ? "w-full"
        ...
        {isOpen && (
          <div className="lg:hidden pb-4 border-t border-gray-200 mt-4 pt-4">
            <div className="flex flex-col space-y-2">
              {navLinks.map((link) => {
                ...
                return (
                  <Link
                    ...
                    className={`font-medium transition-all py-2 px-3 rounded-md ${
                      isActive
              ...
              <a
                href={`tel:${phoneNumber.replace(/\s/g, "")}`}
                className="bg-[#1F3D2B] text-white px-6 py-3 rounded-lg font-semibold hover:bg-[#1a3225] transition-colors text-center mt-2 focus:outline-none focus:ring-2 focus:ring-[#1F3D2B] focus:ring-offset-2"
                onClick={() => setIsOpen(false)}
                aria-label="Call now"
              >
                Call Now
